...
| Service | URL |
|---------|-----|
| **Web app** (Next.js) | [global-conflict-web.vercel.app](https://global-conflict-web.vercel.app/) |
| **REST API** (Express) | [global-conflict-app.onrender.com](https://global-conflict-app.onrender.com/) |

## Tech stack & hosting

| Layer | Technologies | Platform |
|--------|----------------|----------|
| **Frontend** | Next.js (App Router), React 19, Tailwind CSS, next-themes, dayjs | Hosted on **[Vercel](https://vercel.com)** |
| **Backend** | Node.js, Express 5, Mongoose, NewsAPI / optional NewsData.io & OpenAI summaries, `node-cron` ingestion | Hosted on **[Render](https://render.com)** (Node Web Service; root directory `global-conflict-updates`) |
| **Database** | MongoDB (document store for updates & events) | **[MongoDB Atlas](https://www.mongodb.com/cloud/atlas)** (connection via `MONGODB_URI` on the API host) |

The browser talks to the API using `NEXT_PUBLIC_API_BASE_URL` (set in Vercel). The API must list the Vercel origin in `FRONTEND_ORIGIN` for CORS.
...
2. **Production environment variables** (Vercel → Project → Settings → Environment Variables). Use at least:
   ...
   | Name | Value |
   |------|--------|
   | `NEXT_PUBLIC_API_BASE_URL` | Public HTTPS origin of your deployed API (no trailing slash), e.g. `https://global-conflict-app.onrender.com` |
...
3. **Allow CORS on the API** — The Express app only accepts origins listed in `FRONTEND_ORIGIN`. After you have a Vercel URL (and any custom domain), set on the **API host**, for example:
   ...
   `FRONTEND_ORIGIN=http://localhost:3001,https://global-conflict-web.vercel.app`
...
The API itself is not deployed by this step. This project’s API runs on **Render** (see [Production](#production)); use the same env vars as local (`MONGODB_URI`, `NEWS_API_KEY`, `FRONTEND_ORIGIN`, etc.).
